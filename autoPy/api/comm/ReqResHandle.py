...
1.读取报文中字段的值
    ...
    1.2 读取json格式报文文字的值：使用节点路径的方式实现，根据节点路径可能存在列表，值可能存在多个，按不同路径及值存为字典
2.更新字段值
...
def xml_get_field_value(body,field,**value_dict):
    ...
    :return: 值字典，field单个匹配，值为字符串;多个匹配，值为列表
    '''
    #忽略大小写
...
def json_get_field_value(data_dict,field,value_dict=None):
    ...
    class ReqResHandle:
        def __init__(self,data_type,data_msg,fields,value_dict=None):
            ...
            :param fields: 字段值，支持多字读取方式，字段间逗号隔开;或传入列表、元组
                            json格式：节点路径，支持指定列表索引位置的方式（如：a.b[1].c[2].d),
                            也支持按节点提取所有如：a.b.c.d
            :param value_dict:以字典键值对保存字段值（注：当单个字段对应多个值时，xml以列表方式返回，
                            json则返回多重嵌套字典）
            '''
